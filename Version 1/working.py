...
def segment_colour(frame): 
    # Change this
    # Image Analysis : Returns only the red colors in the frame
    hsv_roi = cv2.cvtColor(frame, cv2.cv.CV_BGR2HSV)
    mask_1 = cv2.inRange(hsv_roi, np.array(
        [160, 160, 10]), np.array([190, 255, 255]))
...
for image in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if w*h < 10:
        ...
        centre_x = x+((w)/2)
        centre_y = y+((h)/2)
        cv2.circle(frame, (int(centre_x), int(centre_y)),
                    3, (0, 110, 255), -1)
        # Why these lines and constants
        centre_x = centre_x - 80
    ...
    if found == 0:
        ...
        if area < initial:
            if distanceC < 10:
                # If ball is too far but it detects something in front of it, then it avoids it and reaches the ball.
                if distanceR >= 8:
                    rightturn()
                    time.sleep(0.00625)
                    stop()
            ...
            if area < initial2:
                ...
                # Edit to get a ML model here to get the direction
                # -----------
...
# Random values to look for
# initial = 400, initial2 = 6700
# Raspberry Pi camera imaginary axis?
# RESOLUTION 1024, 768 CHANGE
